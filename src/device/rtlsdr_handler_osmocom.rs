...
#[inline]
fn resolve_software_agc_strategy(
    gain_mode: &GainMode,
    driver_mode_available: bool,
) -> (bool, bool) {
    match gain_mode {
        GainMode::Software => (false, true),
        GainMode::Driver if driver_mode_available => (true, false),
        GainMode::Driver => (false, true),
        GainMode::Hardware | GainMode::Manual(_) => (false, false),
    }
}
...
pub enum GainMode {
    ...
    Software,
    /// Driver AGC: use the old-dab driver's native gain control when available.
    Driver,
...
/// Helper: enable the old-dab driver's native software AGC mode.
/// DABstar drives the default RTL path this way: tuner gain mode `2` delegates
/// the step control to the modified old-dab librtlsdr instead of re-implementing
/// it in the application.
unsafe fn ffi_set_driver_software_agc(dev: *mut rtlsdr_sys::rtlsdr_dev_t) -> bool {
    rtlsdr_sys::rtlsdr_set_agc_mode(dev, 0);
    rtlsdr_sys::rtlsdr_set_tuner_gain_mode(dev, 2) == 0
}
    ...
    pub fn restart_reader(&mut self) -> bool {
        ...
        self.worker_handle = Some(thread::spawn(move || {
            if config.offset_tuning {
                info!("Offset tuning requested on osmocom backend");
            }
            if !config.iq_correction {
                info!("IQ correction disabled on osmocom backend");
            }
            ...
            let driver_mode_available = cfg!(feature = "rtl-sdr-old-dab");
            let (native_driver_sagc_requested, app_sagc_requested) =
                resolve_software_agc_strategy(&config.gain_mode, driver_mode_available);
            let (initial_gain_idx, mut sagc_enabled, hardware_agc) = match config.gain_mode {
                GainMode::Software => {
                    let idx = (gains.len() - 1) * 50 / 100;
                    info!(
                        "Software AGC (application SAGC) enabled, starting at {:.1} dB (50%)",
                        gains[idx] as f32 / 10.0
                    );
                    (idx, app_sagc_requested, false)
                }
                GainMode::Driver => {
                    let idx = (gains.len() - 1) * 50 / 100;
                    if native_driver_sagc_requested {
                        info!(
                            "Driver AGC enabled via old-dab RTL driver, starting near {:.1} dB",
                            gains[idx] as f32 / 10.0
                        );
                    } else {
                        warn!(
                            "Driver AGC requested but unavailable on this backend; falling back to application SAGC"
                        );
                    }
                    (idx, app_sagc_requested, false)
                }
            ...
            } else if native_driver_sagc_requested {
                let native_ok = unsafe { ffi_set_driver_software_agc(dev) };
                if !native_ok {
                    warn!(
                        "old-dab native software AGC mode was not accepted; falling back to application SAGC"
                    );
                    sagc_enabled = true;
                    let r = unsafe { ffi_set_manual_gain(dev, gains[initial_gain_idx]) };
                    if !r {
                        warn!("set_tuner_gain failed for fallback SAGC initial gain");
                    }
                }
            } else {
                let r = unsafe { ffi_set_manual_gain(dev, gains[initial_gain_idx]) };
            ...
            loop {
                ...
                if n_read == 0 {
                    tracing::debug!("rtlsdr_read_sync returned 0 bytes; retrying");
                    continue;
                }
...
#[cfg(test)]
mod tests {
    use super::{resolve_software_agc_strategy, GainMode};

    #[test]
    fn software_mode_uses_application_sagc_only() {
        let (driver_sagc, app_sagc) = resolve_software_agc_strategy(&GainMode::Software, true);
        assert!(!driver_sagc);
        assert!(app_sagc);
    }

    #[test]
    fn driver_mode_uses_driver_agc_without_app_sagc() {
        let (driver_sagc, app_sagc) = resolve_software_agc_strategy(&GainMode::Driver, true);
        assert!(driver_sagc);
        assert!(!app_sagc);
    }

    #[test]
    fn driver_mode_falls_back_to_app_sagc_when_unavailable() {
        let (driver_sagc, app_sagc) = resolve_software_agc_strategy(&GainMode::Driver, false);
        assert!(!driver_sagc);
        assert!(app_sagc);
    }
}
